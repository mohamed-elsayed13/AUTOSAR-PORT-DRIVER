...
void Port_Init(const Port_ConfigType* ConfigPtr)
{
   ...
   g_Port_ConfigPtr = ConfigPtr;     /*points to the 1st address of the array of structure*/
   g_Port_Status = PORT_INITIALIZED;
   uint8 pin;              /* index to configure each pin (43)*/
    
  #if (DIO_DEV_ERROR_DETECT == STD_ON)

  /* check if the input configuration pointer is not a NULL_PTR */
  if (NULL_PTR == g_Port_ConfigPtr)
  {
  ...
  #endif
  for(pin=PORTA_PA0;pin<PORT_CONFIGURED_CHANNLES;pin++){
  /*Choosing the required Port number*/
  switch(g_Port_ConfigPtr->Channels[pin].port_num)
    {
  ...
  /* Enable clock for PORT and allow time for clock to start*/
    SYSCTL_REGCGC2_REG |= (1<<g_Port_ConfigPtr->Channels[pin].port_num);
    delay = SYSCTL_REGCGC2_REG;
  
    /*Unlocking locked pins to be able to configure and use them*/
    if( ((g_Port_ConfigPtr->Channels[pin].port_num == PORT_D) && (g_Port_ConfigPtr->Channels[pin].pin_num == NMI_PIN2)) || ((g_Port_ConfigPtr->Channels[pin].port_num == PORT_F) && (g_Port_ConfigPtr->Channels[pin].pin_num == NMI_PIN1)) ) /* PD7 or PF0 */
    {
        *(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_LOCK_REG_OFFSET) = UNLOCK_VALUE;                     /* Unlock the GPIOCR register */   
        SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_COMMIT_REG_OFFSET) , g_Port_ConfigPtr->Channels[pin].pin_num);  /* Set the corresponding bit in GPIOCR register to allow changes on this pin */
    }

/*Preventing any actions to be done upon JTAG pins*/
     else if(  
            ( (g_Port_ConfigPtr->Channels[pin].port_num == PORT_C) && (g_Port_ConfigPtr->Channels[pin].pin_num == JTAG_PIN1) )||
            ( (g_Port_ConfigPtr->Channels[pin].port_num == PORT_C) && (g_Port_ConfigPtr->Channels[pin].pin_num == JTAG_PIN2) )||
            ( (g_Port_ConfigPtr->Channels[pin].port_num == PORT_C) && (g_Port_ConfigPtr->Channels[pin].pin_num == JTAG_PIN3) )||
            ( (g_Port_ConfigPtr->Channels[pin].port_num == PORT_C) && (g_Port_ConfigPtr->Channels[pin].pin_num == JTAG_PIN4) ))
      {
   ...
   *And if Input pin -> configure the internal resistor Pull-up/Pull-down*/
    ...
    if(ConfigPtr->Channels[pin].Pin_mode == PORT_DIO_MODE)
            {
                ...
                *(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_PCTL_REG_OFFSET) &= ~(0x0000000F << ((ConfigPtr->Channels[pin].pin_num) * 4));     /* Clear the PMCx bits for this pin */
                /*If output pin*/
                if(ConfigPtr->Channels[pin].Pin_direction == PORT_PIN_OUT)
                {
                    ...
                    if(ConfigPtr->Channels[pin].Pin_level_initial_value== STD_HIGH)
                    {
                ...
                /*If Input pin*/
                else if(ConfigPtr->Channels[pin].Pin_direction == PORT_PIN_IN)
                {
    ...
    /*Configure Analog mode used for ADC only*/
      ...
      else if(ConfigPtr->Channels[pin].Pin_mode == PORT_ALTERNATE_FUNCTION_ADC_MODE)
        {
            CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIGITAL_ENABLE_REG_OFFSET) , ConfigPtr->Channels[pin].pin_num);    /* Clear the corresponding bit in the GPIODEN register to disable digital functionality on this pin */
            ...
            {
                SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIR_REG_OFFSET) , ConfigPtr->Channels[pin].pin_num);                /* clear the corresponding bit in the GPIODIR register to configure it as input pin */              
            }
            
            SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_ANALOG_MODE_SEL_REG_OFFSET) , ConfigPtr->Channels[pin].pin_num);     /* Set the corresponding bit in the GPIOAMSEL register to enable analog functionality on this pin */
            
        }

/*If any other alternative functionality between DIO Id and ADC id*/
        else if((ConfigPtr->Channels[pin].Pin_mode > PORT_DIO_MODE) && (ConfigPtr->Channels[pin].Pin_mode < PORT_ALTERNATE_FUNCTION_ADC_MODE))
        {
            CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_ANALOG_MODE_SEL_REG_OFFSET) , ConfigPtr->Channels[pin].pin_num);            /* Clear the corresponding bit in the GPIOAMSEL register to disable analog functionality on this pin */
            SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_ALT_FUNC_REG_OFFSET) , ConfigPtr->Channels[pin].pin_num);                     /* enable Alternative function for this pin by Setting the corresponding bit in GPIOAFSEL register */
            *(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_PCTL_REG_OFFSET) &= ~(0x0000000F << ((ConfigPtr->Channels[pin].pin_num) * 4));         /* Clear the PMCx bits for this pin */
            *(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_PCTL_REG_OFFSET) |= (ConfigPtr->Channels[pin].Pin_mode<< ((ConfigPtr->Channels[pin].pin_num)* 4));  /* Set the PMCx bits for this pin to the selected Alternate function in the configurations */
            SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIGITAL_ENABLE_REG_OFFSET) , ConfigPtr->Channels[pin].pin_num);               /* Set the corresponding bit in the GPIODEN register to enable digital functionality on this pin */
  ...
  /*Check if the API service called prior to module initialization*/
    ...
    if (g_JTAG_flag == STD_LOW)
	{
         /*The function Port_RefreshPortDirection shall exclude those port pins from refreshing that are configured as ?pin direction changeable during runtime*/ 
          if(g_Port_ConfigPtr->Channels[Pin].Pin_directionChange == PIN_DIRECTION_CHANGEABLE_OFF )
		{
                  switch (g_Port_ConfigPtr->Channels[Pin].Pin_directionChange)
                  {
                  case PORT_PIN_OUT:
                       /* Set the corresponding bit in the GPIODIR register to configure it as output pin */
			SET_BIT(*(volatile uint32 *)((volatile uint8 *)Port_refreshDirectionPtr + PORT_DIR_REG_OFFSET) ,g_Port_ConfigPtr->Channels[Pin].pin_num);
                  break;
                  
                  case PORT_PIN_IN:
                       /* Clear the corresponding bit in the GPIODIR register to configure it as input pin */
			CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)Port_refreshDirectionPtr + PORT_DIR_REG_OFFSET) , g_Port_ConfigPtr->Channels[Pin].pin_num);
		  break;
